...
export default class FileDetails extends HTMLElement {
  ...
  connectedCallback() {
    ...
    subscribe("CHANGE_DIRECTORY", ({ node, path }) =>
      this.changeDirectoryHandler(node, path)
    );
  ...
  render() {
    ...
    const nodeKeys = Object.keys(node).filter(
      (key) => !attributesToExclude.includes(key)
    );
    this.innerHTML =
      `
      <h2>
      ...
      this.detailSection(nodeKeys) +
      `${
        node.node_type !== "COLLECTION"
          ? `<dt>Location</dt><dd class="location-detail">${path}</dd>`
          : ""
      }</dl>
        ${
          contentUrl
            ? `<ui5-button design="Default" id="download-file-btn">Download</ui5-button>`
            : ""
        }
      </div>
    ...
    this.tabContainer.addEventListener(
      "tab-select",
      this.tabSelectHandler.bind(this)
    );
  }
      ...
      if (node[key] !== null && node[key] !== "") {
        if (key === "uploaded_by") {
          ...
          str += `
                      ...
                      <dd style="display: inline-block;">${node[key].slice(
                        0,
                        8
                      )}</dd>
                      <div class="tooltipElement">
                        <span class="tooltiptext" id="hover-text-${hashString.slice(
                          hashString.length - 1
                        )}">Copy Text</span>
                      <div class="copy-hash-btn" data="${
                        node[key]
                      }" ><img src="/vault/static/favicon/copyIcon.png" alt="Copy Icon" class="copy-img"></div>
                      </div>
                      `;
        ...
        <ui5-card class="small activity-card">
          <div class="activity-content">
              ...
              <ui5-label class="activity-title-desc">${
                k === "Total Size" ? humanBytes(event[k]) : event[k]
              }</ui5-label>
              </div>`
                    : ""
                )
                .join("")}
              <ui5-link class="view-details-link activity-title-desc" href="${
                event["Link"]
              }" target = "_blank">View Details</ui5-link>
          </div>
      </ui5-card>`;
        });
      } else {
