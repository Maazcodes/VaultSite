...
export default class FileTreeNavigator extends HTMLElement {
  ...
  connectedCallback() {
    ...
    subscribe(
      ...
      this.nodeChildrenResponseHandler.bind(this)
    );
    ...
    if (node.id === this.orgId || !document.getElementById("treeDynamic")) {
      ...
      this.innerHTML = `
        <ui5-tree id="treeDynamic">
          <ui5-tree-item text="Collections" id="${
            this.orgId
          }" has-children expanded selected>
            ${this.props.nodes
      ...
      if (!nodeElement) {
        ...
        for (let index = 0; index < nodePathArray.length; index++) {
          ...
          if (document.getElementById(String(NodeId))) {
            ...
            this.CreateTreeViewElements(
              this.parentChildDictionary[NodeId] ||
                this.parentChildDict[NodeId],
              ParentNode
            );
          }
        }
  ...
  _createTreeViewElement(node, parentNode) {
    ...
    if (node.node_type === "FOLDER") {
      newItem.setAttribute(
        "path",
        String($(parentNode).attr("path") || "") + "/" + String(newItem.text)
      );
    } else {
  ...
  CreateTreeViewElements(nodesArray, parentNode) {
    ...
    for (let index = 0; index < nodesArray.length; index++) {
      ...
      if (
        childNode.node_type !== "FILE" &&
        !document.querySelector(`ui5-tree-item[id='${childNode.id}']`)
      ) {
  ...
  nodeChildrenResponseHandler({ childResponse, nodeId, action }) {
    ...
    if (nodesChildrenResponse.length > 0) {
      ...
      parentElement.innerHTML = `${nodesChildrenResponse
        .filter((node) => node.node_type !== "FILE")
        .map(
          (node) =>
            `<ui5-tree-item text="${node.name}" path="${$(parentElement).attr(
              "path"
  ...
  nodeRenameResponseHandler({ node, newName, error }) {
    ...
    this.querySelector(`ui5-tree-item[id="${node.id}"]`).setAttribute(
      "text",
      newName
    );
  }
