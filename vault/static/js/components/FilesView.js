...
class Conductor {
  constructor({ basePath, appPath, path, node, orgId }) {
    ...
    subscribe("CHANGE_DIRECTORY_REQUEST", (message) =>
      this.changeDirectoryRequestHandler(message)
    );

    subscribe("CREATE_COLLECTION_REQUEST", (message) =>
      this.createCollectionRequestHandler(message)
    );

    subscribe("CREATE_FOLDER_REQUEST", (message) =>
      this.createFolderRequestHandler(message)
    );

    subscribe("NODE_CHILDREN_REQUEST", (message) =>
      this.nodeChildrenRequestHandler(message)
    );

    subscribe("NODE_RENAME_REQUEST", (message) =>
      this.nodeRenameRequestHandler(message)
    );

    subscribe("NODES_DELETE_REQUEST", (message) =>
      this.nodesDeleteRequestHandler(message)
    );

    subscribe("NODE_REQUEST", (message) => this.nodeRequestHandler(message));
  ...
  async nodeRequestHandler({ nodeId, action }) {
    ...
    const nodeResponse = await this.api.treenodes.get(null, {
      id: nodeId,
      limit,
      ordering,
    });
    publish("NODE_RESPONSE", {
      nodeResponse: nodeResponse.results,
      action: action,
    });
  }
  ...
  async nodeChildrenRequestHandler({ nodeId, action }) {
    const { limit, ordering } = this.state.childQuery;
    const nodeChildrenResponse = await this.api.treenodes.get(null, {
      parent: nodeId,
      limit,
      ordering,
    });
    publish("NODE_CHILDREN_RESPONSE", {
      childResponse: nodeChildrenResponse.results,
      nodeId: nodeId,
      action: action,
    });
  }

  async changeDirectoryRequestHandler(
    { nodeId, node, path },
    updateHistory = true
  ) {
    ...
    const childNodesResponse = await this.api.treenodes.get(null, {
      parent: node.id,
      limit,
      ordering,
    });
...
export default class FilesView extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.querySelector("#file-tree-navigator-container").appendChild(
      this.fileTreeNavigator
    );
    ...
    this.querySelector("#file-details-container").appendChild(this.fileDetails);
