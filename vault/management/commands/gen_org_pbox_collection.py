...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="WARNING: Overwrites the existing petabox collection set for the specified Organization (Don't do this)",
        )
    ...
    def handle(self, *args, **options):
        ...
        if organization:
            ...
            if item.exists:
                print(
                    f"Item already exists {options['pbox_collection_name']}. Skipping"
                )
                sys.exit(-1)
            ...
            if organization.pbox_collection is not None:
                print(
                    f"Organization:{organization.id} already has a petabox collection set {organization.pbox_collection}."
                )
                if options["force"]:
                    print(
                        "!!!WARNING!!! force overwrite has been set. Organization's existing petabox collection will be overwritten"
                    )
                    print(
                        f"\tOrganization.pbox_collection is currently {organization.pbox_collection}"
                    )
                    print(
                        f"\tOrganization.pbox_collection will become {options['pbox_collection_name']}"
                    )
            ...
            responses = item.upload(
                files="vault/static/vault-logo-3.png", metadata=metadata
            )
            if len(responses) == 1:
                print(f"S3 upload response: {responses[0].status_code}")
                if responses[0].status_code == 200:
                    print(
                        f"Adding pbox collection to organization id:{organization.id} - {organization.name}"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"Error saving pbox_collection to organization {organization.id} - {e}"
                        )
                        sys.exit(-1)
            else:
